...
async def generate_chat_token(request: Request, requirements: GenerateTokenRequest, generate_token_adapter: GenerateAccessTokenAdapter = Depends(get_generate_token_adapter)) -> None:
    token_requirement = requirements.token_requirement
    user_role = requirements.role
    ttl: int = requirements.lifetime

    await generate_token_adapter.generate_and_save_token(token_requirement, user_role, ttl)
